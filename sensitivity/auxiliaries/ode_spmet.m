...
function [x_dot,varargout] = ode_spmet(t,x,data,p,pscale)
...
Unref = refPotentialAnode(p,theta_n,pscale);
Upref = refPotentialCathode(p,theta_p,pscale);
...
[Unb,~,dUnbdT] = refPotentialAnode(p, SOC_n, pscale);
[Upb,~,dUpbdT] = refPotentialCathode(p, SOC_p, pscale);
