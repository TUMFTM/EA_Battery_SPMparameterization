...
for i=1:size(P,2)-1
    ...
    % Crawl data
    I = P(i).I(1:t_step:end)*B_A.s_pos; % Inverse and scale for anode in full cell simulation setup    
    ...
    % Prep fitting
    p0 = [1, ... % Manipulation factor of diffusion coeff for solid in pos. electrode
          1];    % Manipulation factor of reaction rate in pos. electrode
    ...
    hold on;
    plot(t, U, 'k');
    plot(t, real(Upref+eta_p+eta_sei_p), 'Color', [152 198 234]./255);
    ...
    RMSE = sqrt(mean((U' - real(Upref+eta_p+eta_sei_p)).^2));  % Root Mean Squared Error
 
    subplot(4,10,i);
    hold on;
    plot(t, real(Upref+eta_p), 'Color', [227 114 34]./255);
